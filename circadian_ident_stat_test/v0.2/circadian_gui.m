function circadian_gui
    ...
    genes = sort(sce.g); % Assuming sce.g is a cell array or similar
    
    % --- Start Gene Input Controls ---
    uicontrol('Style', 'text', ...
              'Position', [45, 170, 95, 20], ...
              'String', 'Select OR Write Gene:');
    
    % 2. Dropdown Menu (for selection from the list)
    hGeneDropdown = uicontrol('Style', 'popupmenu', ...
                              'Position', [145, 170, 100, 20], ...
                              'String', genes, ...
                              'Value', 1);
    % 3. Editable Text Box (The user's direct input option, placed "aside")
    hGeneEdit = uicontrol('Style', 'edit', ...
                          'Position', [255, 170, 90, 20], ...
                          'String', ''); % Start with an empty string
    % --- End Gene Input Controls ---
    ...
    function plotGeneCallback(~, ~)
        ...
        % --- CORRECTED LOGIC: Select gene from edit box OR dropdown ---
        typedGene = get(hGeneEdit, 'String'); % Get value from the edit box
        
        if ~isempty(typedGene)
            gene = typedGene; % Use typed gene if it's not empty
        else
            gene_options = get(hGeneDropdown, 'String'); % Get all options from dropdown
            gene_value = get(hGeneDropdown, 'Value');   % Get the index of the selected option
            gene = gene_options{gene_value};     % Retrieve selected gene name
        end
        % --- END CORRECTED LOGIC ---
