...
%[ScoreV, T] = sc_cellcyclescore(X, g);
% Positive values of potency are higher in differentiation, neg is low.
% Annealing will use negative of positive values to maximize
cell_state = -1*sc_potency(X, sce.g, 'mouse');
...
g = sce.g(idg);
ng = length(g);
% Subset count matrix to only requested genes
X = X(idg, :);
...
Xko_mnn = adjX_mat_construct_sparse_cust_idx(Xko, 'mnn', K0); % large K gets more mnn
...
% Compute the nearest neighboars of target genes only
Xwt_mnn = adjX_mat_construct_sparse_cust_idx(Xwt, 'mnn', K0); % large K gets more mnn
...
if plotit 
     ...
     colormap("turbo"); % Consider using a divergent colormap like 'coolwarm' for differences
end
...
% Sub network and nodes contribution
% Q1 contains mnn, Xdiff, Vdiff and net target (if more genes than pathways
% contributes to the cost function.)
sub_Q_net = -Q1(idx, idx);
sub_Qv = -Q1*result.BestX;
sub_Qv = sub_Qv(idx);
...
% We have to use the original network, since many terms were produced to
...
edge_display_threshold = prctile(non_nan_abs_diffs, 0); % Only show top 20% of abs diffs
