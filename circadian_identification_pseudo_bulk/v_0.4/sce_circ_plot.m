function sce_circ_plot(sce, tmeta, cust_cells, plot_type, period12 )
    ...
    % This function computes all genes
    cust_gene = [];
    % Always remove unconfident genes
    if plot_type == 3 
        rm_low_conf = false;
    else
        rm_low_conf = true; 
    ...
    switch plot_type
        ...
        case 3
            ...
            gidx = find(founds == 1)';
            path = strcat(cust_cells,'_classic_circadian');
            disp("Classical circadian gene results will be stored in "+path)
            mkdir(path);
            for i = gidx
                f = figure('visible','off');
                fval = T1.Amp(i).*cos( 2*pi*( tval - T1.Acrophase(i))./T1.Period(i) ) + T1.Mesor(i);
