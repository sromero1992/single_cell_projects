function [T1, confident_idx] = gen_pvalues( T1, mode, err_type)
    ...
    switch mode
        case 1
            ...
            y_logMAE = pdf(pd_logMAE_skewnormal, x_logMAE); % Evaluate PDF at x_logMAE
            figure;
            histogram(log_func, 'Normalization', 'pdf'); % Plot histogram of log_func
            hold on;
            plot(x_logMAE, y_logMAE, 'r-', 'LineWidth', 2); % Plot fitted skew normal distribution
            hold off;
            title('Histogram of Log-transformed MAE and Fitted Skew Normal Distribution');
            xlabel('Log-transformed Mean Absolute Relative Error (Log(MAErel+1))');
