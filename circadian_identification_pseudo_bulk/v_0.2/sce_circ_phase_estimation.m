function [T1, T2] = sce_circ_phase_estimation(sce, tmeta,  rm_low_conf, period12, ...
                                    cust_genes, cust_cells)
    % sce_circ_phase_estimation computes the sine/cosine fit and gets its
    % error, building a kernel distribution for pvalues contruction.
    % retrieves the confident fitted genes and if they have any tipe of
    % failure. Failure = 1 indicates 0 expression, failure = 3 indicates
    % more than 50% error in directions per timepoint (too noisy) and
    % failure = 2 if not enough cells for at least one time point was found
    % INPUT:
    % sce =========> Single cell experiment object
    % tmeta =======> Metatable containing circadian information (see usage)
    % rm_low_conf => Remove low confidence pvalue (pval > 0.05)
    % period12 ====> Use period 12 (true) or 24 (false)
    % cust_genes ==> Custom gene list to obtain circadian properties
    % cust_cells ==> Custom cell type to compute
    % OUTPUT: 
    % T1 ==========> Table containing circadian information
    % T2 ==========> Table containing mean expression by time point (ZTs)
    % USAGE:
    ...
    % [T1, T2] = sce_circ_phase_estimation(sce, tmeta, false, [], cust_genes, cust_cells)
    ...
    if nargin < 5 || isempty(cust_genes); cust_genes = []; end
    ...
    if ~right_input 
        disp('Your metadata table does not match sce object batches');
        disp('Please, ensure that same number of batches match old_input');
        return;
    end
    ...
    for icell_type = 1:ncell_types   
        ...
        if ~isempty(cust_cells)
            if ~ismember(cell_type, cust_cells) 
                disp("Selected celltype not available: "+cust_cells);
                continue; 
            end
        end
        ...
        disp("Size of count matrix"size(X))
        ...
        if isempty( cust_genes)
            disp("Circadian analysis for all genes");
            gene_list = sce_sub.g;
        else
            disp("Circadian analysis for custom genes");
            nx = size(cust_genes, 1);
            if nx == 1; cust_genes = cust_genes'; end
            [lic, ~] = ismember(cust_genes, sce_sub.g);
            cust_genes = cust_genes(lic);
            gene_list = cust_genes;
        end
        ...
        parfor igene = 1:ngene
            ...
            [tmp_acro(igene), tmp_amp(igene), tmp_T(igene), tmp_mesor(igene)] = ...
                           estimate_phaseR(R, tint, period12);
        ...
        % Remove 50% errors in direction or not same direction in end points
        % Let user be aware of it only
        ...
        ftable_name = strcat(fname,"_circadian_analysis.csv");
        writetable(T1,ftable_name);
        
        ftable_name = strcat(fname,"_circadian_ZTs.csv");
