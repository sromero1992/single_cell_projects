# =============================================================================
# scRNA-seq PIPELINE - SCRIPT 1: DATA PROCESSING ENGINE (v7.8 - with Memory Management)
#
# PURPOSE:
# This script robustly processes each sample with a checkpoint system and explicit
# memory management, saving a complete intermediate object. It can be safely
  # It then merges all samples for final processing.
  #
  # =============================================================================
...
SCEVAN_DIR <- file.path(OUTPUT_DIR, "scevan_per_sample_results") # Dedicated SCEVAN output/checkpoint directory
...
DOUBLET_RATE <- 0.08; DF_PK_RANGE_MIN <- 0.01; DF_PK_RANGE_MAX <- 0.15; DF_PK_FALLBACK <- 0.09
...
RUN_SCEVAN <- TRUE
DPI_SETTING <- 300
# --- Probe/KO Gene Integration Parameters ---
ADD_PROBE_DATA <- TRUE
PROBE_MAPPING <- list('Nr4a1|dbf6af3'='Exon 3 (KO Target)', 'Nr4a1|99a0eaa'='Exon 5 (KO Target)', 'Nr4a1|03c248a'='Exon 7')
PROBES_FOR_CUSTOM_SUM <- c('Nr4a1|dbf6af3', 'Nr4a1|99a0eaa')
# --- SCEVAN Parameters ---
SCEVAN_ORGANISM <- "mouse"; SCEVAN_N_CORES <- 4; SCEVAN_SUBCLONES <- TRUE; SCEVAN_PLOTTREE <- FALSE
# =============================================================================
...
# --- 2.1: Process or Load Each Sample Individually (with Checkpoints) ---
...
for (i in 1:nrow(metadata)) {
  ...
  if (file.exists(checkpoint_file)) {
    message(paste("... Checkpoint found for", sample_id, ". Loading from disk."))
    seurat_obj <- readRDS(checkpoint_file)
  } else {
    message(paste("... No checkpoint for", sample_id, ". Processing from scratch."))
    ...
    if (!file.exists(rna_h5_path)) { warning(paste("RNA H5 file not found:", sample_id)); next }
    ...
    if (ADD_PROBE_DATA) {
      ...
      if (file.exists(probe_h5_path)) {
        message("    -> Reading probe data and creating custom 'Nr4a1-cust' feature...")
        probe_matrix <- Read10X_h5(probe_h5_path)
        if (is.list(probe_matrix) && "Probe Barcode" %in% names(probe_matrix)) { probe_matrix <- probe_matrix[['Probe Barcode']] }
        common_cells <- intersect(colnames(counts_matrix), colnames(probe_matrix)); custom_sum_counts <- numeric(length(common_cells)); names(custom_sum_counts) <- common_cells
        probes_found <- intersect(PROBES_FOR_CUSTOM_SUM, rownames(probe_matrix))
        if(length(probes_found) > 0) { summed_vector <- Matrix::colSums(probe_matrix[probes_found, common_cells, drop=F]); custom_sum_counts[names(summed_vector)] <- summed_vector }
        custom_row <- Matrix::Matrix(0,1,ncol(counts_matrix),sparse=T); colnames(custom_row)<-colnames(counts_matrix); rownames(custom_row)<-"Nr4a1_cust"
        custom_row[1, names(custom_sum_counts)] <- custom_sum_counts
        counts_matrix <- rbind(counts_matrix, custom_row)
      } else { warning(paste("    -> Probe H5 not found for", sample_id)) }
    }
    ...
    if ("Condition" %in% colnames(seurat_obj@meta.data) && "Sex" %in% colnames(seurat_obj@meta.data)) { seurat_obj$group <- paste(seurat_obj$Condition, seurat_obj$Sex, sep = "_") }
    if (!is.null(probe_matrix)) {
      for (probe_id in names(PROBE_MAPPING)) { if (probe_id %in% rownames(probe_matrix)) {
        col_name_clean <- gsub("[\\s\\(\\)\n]+", "_", PROBE_MAPPING[[probe_id]]); col_name_clean <- gsub("_$", "", col_name_clean)
        col_name_final <- paste0("Nr4a1_", col_name_clean, "_probe_count")
        seurat_obj <- AddMetaData(seurat_obj, metadata = probe_matrix[probe_id, ], col.name = col_name_final)
      }}}
    
    if (RUN_SCEVAN) {
      tryCatch({
        message("    -> Running SCEVAN...")
        scevan_res_df <- pipelineCNA(as.matrix(GetAssayData(seurat_obj, layer="counts")), par_cores=SCEVAN_N_CORES, SUBCLONES=SCEVAN_SUBCLONES, plotTree=SCEVAN_PLOTTREE, organism=SCEVAN_ORGANISM, output_dir=sample_scevan_dir)
        if (!is.null(scevan_res_df)) { seurat_obj <- AddMetaData(seurat_obj, metadata = scevan_res_df); message("    -> SCEVAN metadata added to Seurat object.") }
      }, error = function(e) { message(paste("    -> !!! SCEVAN failed for", sample_id, "!!! Error:", e$message)) })
    }
    ...
    message("    -> Saving complete intermediate object to checkpoint file.")
    saveRDS(seurat_obj, file = checkpoint_file)
    
    # --- Clean up memory before next sample --- <<< NEW MEMORY MANAGEMENT
    message("    -> Cleaning up memory before next iteration...")
    rm(counts_matrix, seurat_obj) # Remove the large objects from this iteration
    if (!is.null(probe_matrix)) { rm(probe_matrix) }
    gc() # Explicitly run garbage collection
    
    # Reload the object to add to the list - this ensures a clean memory state
    seurat_obj <- readRDS(checkpoint_file)
  }
  seurat_objects_list[[sample_id]] <- seurat_obj
}

# --- 2.2: Merge All Processed Samples ---
...
# --- 2.3: Post-Merge QC and Diagnostic Plotting ---
message("\n--- Step 2.3: Post-Merge QC and Plotting ---")
# ... (The rest of the script is unchanged and continues from here) ...
...
# --- 2.4: Doublet Detection with Diagnostic Plotting ---
message("\n--- Step 2.4: Doublet Detection and Plotting ---")
...
for (i in seq_along(data_list)) {
  ...
  if (is.na(final_pk) || final_pk < DF_PK_RANGE_MIN || final_pk > DF_PK_RANGE_MAX) {
    bcmvn_filtered <- bcmvn[bcmvn$pK > DF_PK_RANGE_MIN & bcmvn$pK < DF_PK_RANGE_MAX, ]; if (nrow(bcmvn_filtered) > 0 && any(is.finite(bcmvn_filtered$BCmetric))) { final_pk <- bcmvn_filtered$pK[which.max(bcmvn_filtered$BCmetric)] } else { final_pk <- DF_PK_FALLBACK }
  }
  pk_plot <- ggplot(bcmvn, aes(x = pK, y = BCmetric, group = 1)) + geom_line() + geom_point() + geom_vline(xintercept = final_pk, linetype = "dashed", color = "red") + ggtitle(paste0("pK Finder for ", sample_name), subtitle = paste0("Final Selected pK = ", final_pk)) + theme_minimal()
...
data <- NormalizeData(data, verbose = F) %>% FindVariableFeatures(verbose = F) %>% ScaleData(verbose = F) %>% RunPCA(npcs = N_PCS_TO_USE, verbose = F, reduction.name = "pca_temp_doublets") %>% RunUMAP(dims = 1:N_PCS_TO_USE, reduction = "pca_temp_doublets", reduction.name = "umap_temp_doublets", verbose = F)
p_doublets <- DimPlot(data, reduction = "umap_temp_doublets", group.by = "Doublet_Status", cols = c("Singlet" = "grey", "Doublet" = "red")) + ggtitle("Doublet Visualization Before Removal")
ggsave(file.path(OUTPUT_DIR, "01c_DIAGNOSTIC_doublet_visualization.png"), plot = p_doublets, width = 8, height = 7, dpi = DPI_SETTING)
data@reductions$pca_temp_doublets <- NULL; data@reductions$umap_temp_doublets <- NULL
rm(p_doublets); gc()
...
# --- 2.5: Ambient RNA Correction (Optional) ---
if (RUN_DECONTX) {
  message("\n--- Step 2.5: DecontX ---")
  ...
  data <- subset(data, subset = nFeature_RNA >= POST_MIN_GENES & nFeature_RNA <= POST_MAX_GENES & nCount_RNA >= POST_MIN_UMIS & nCount_RNA <= POST_MAX_UMIS & percent.mt <= POST_MAX_MT)
}

# --- 2.6: Processing, Integration, and Clustering ---
message("\n--- Step 2.6: Processing, Integration, and Clustering ---")
...
p1 <- DimPlot(data, reduction = "umap_none", group.by = "SampleID") + ggtitle("Standard PCA (No Harmony)"); p2 <- DimPlot(data, reduction = "umap_harmony", group.by = "SampleID") + ggtitle("Harmony Corrected")
ggsave(file.path(OUTPUT_DIR, "02_DIAGNOSTIC_UMAP_Harmony_vs_NoHarmony.png"), plot = p1 + p2, width = 16, height = 7, dpi = DPI_SETTING)
p3 <- DimPlot(data, reduction = "umap_none", group.by = "clusters_none", label = TRUE) + ggtitle("Clusters: Standard PCA") + NoLegend(); p4 <- DimPlot(data, reduction = "umap_harmony", group.by = "clusters_harmony", label = TRUE) + ggtitle("Clusters: Harmony") + NoLegend()
ggsave(file.path(OUTPUT_DIR, "03_DIAGNOSTIC_UMAP_Cluster_Comparison.png"), plot = p3 + p4, width = 16, height = 7, dpi = DPI_SETTING)

# --- 2.7: Saving Processed Object for Annotation ---
message("\n--- Step 2.7: Saving Processed Object ---")
